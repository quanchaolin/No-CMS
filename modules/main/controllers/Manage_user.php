...
class Manage_user extends CMS_Predefined_Callback_CRUD_Controller {
    ...
    protected function make_crud(){
        ...
        $crud->display_as('login','登录');
        $crud->display_as('subsite','子网站');
        $crud->display_as('module','模块');
        $crud->display_as('group_user','用户组');
    ...
    {
        $input = '<input name="new_password" value="" type="input" class="form-control" placeholder="新密码或留空" />';
    ...
    public function _callback_field_group_user($value, $primary_key){
        ...
        $query = $this->db->select('group_id, group_name')
            ...
            ->limit(20)
            ->get();
        $html = '<select id="field-group_user" name="group_user[]" multiple="multiple" size="8" class="form-control" data-placeholder="选择组">';
        // add old values
