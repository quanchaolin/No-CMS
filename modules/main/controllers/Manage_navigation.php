...
class Manage_navigation extends CMS_Predefined_Callback_CRUD_Controller {
    ...
    protected function make_crud($parent_id = NULL){
        ...
        $crud->display_as('navigation_name','导航名称');
        $crud->display_as('parent_id','上级');
        $crud->display_as('title','菜单名称');
        $crud->display_as('bootstrap_glyph','Bootstrap 字形');
        $crud->display_as('page_title','页面标题');
        $crud->display_as('url','Url');
        $crud->display_as('authorization_id','授权');
        $crud->display_as('active','活动');
        $crud->display_as('index','排序');
        $crud->display_as('is_static','是否静态');
        $crud->display_as('static_content','静态内容');
        $crud->display_as('only_content','只是显示内容');
        $crud->display_as('default_theme','默认主题');
        $crud->display_as('default_layout','默认布局');
        $crud->display_as('notif_url','Notif Url');
        $crud->display_as('children','Children');
        $crud->display_as('hidden','隐藏');
        $crud->display_as('group_navigation','用户组');
        $crud->display_as('custom_style','自定义样式 (CSS)');
        $crud->display_as('custom_script','自定义脚本 (Javascript)');
        $crud->display_as('page_keyword', 'Meta关键词');
        $crud->display_as('description', 'Meta 描述');
        $crud->display_as('page_author', 'Meta 作者');
        $crud->display_as('page_type', 'Meta 类型');
        ...
        $crud->display_as('page_image', 'Meta 图片');
    ...
    public function _callback_column_navigation_name($value, $row)
    {
        ...
        if ($row->active == 0) {
            $html .= '<a href="#" target="'.$target.'" class="navigation_active"><i class="glyphicon glyphicon-eye-open"></i> <span>不活动</span></a>';
        } else {
            $html .= '<a href="#" target="'.$target.'" class="navigation_active"><i class="glyphicon glyphicon-eye-open"></i> <span>活动</span></a>';
        }
        // expand
        if ($can_be_expanded) {
            $html .= ' | <a href="#" class="expand-collapse-children" target="'.$row->navigation_id.'"><i class="glyphicon glyphicon-chevron-up"></i> 折叠</a>';
        }
        $from = '';
        if(isset($_GET['from'])){
            $from .= '?from='.$_GET['from'];
        ...
        if (isset($_SESSION['__mark_move_navigation_id'])) {
            ...
            if ($row->navigation_id == $mark_move_navigation_id) {
                // cancel link
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_navigation/navigation_move_cancel').'"><i class="glyphicon glyphicon-repeat"></i> 撤消</a>';
            } else {
                // paste before, paste after, paste inside
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_navigation/navigation_move_before/'.$row->navigation_id).'"><i class="glyphicon glyphicon-open"></i> 放在前面</a>';
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_navigation/navigation_move_after/'.$row->navigation_id).'"><i class="glyphicon glyphicon-save"></i> 放在后面</a>';
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_navigation/navigation_move_into/'.$row->navigation_id).'"><i class="glyphicon glyphicon-import"></i> 放进</a>';
            }
        } else {
            $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_navigation/navigation_mark_move/'.$row->navigation_id).'"><i class="glyphicon glyphicon-share-alt"></i> 移动</a>';
        }
