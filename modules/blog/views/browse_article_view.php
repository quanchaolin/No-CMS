    ...
    <div class="modal-dialog col-xs-12" style="width:100%!important;">
        ...
        <div class="modal-content" style="width:100%!important;">
            <div class="modal-header">
                ...
                <h4 id="photo-modal-title" class="modal-title" style="display:inline-block;">照片</h4>
                ...
                <form  method="post" action="<?php echo $backend_url; ?>/add/">
                    ...
                    <div class="col-xs-12 col-md-6" style="margin-bottom:10px;">
                        <select id="new_article_status" name="status" class="form-control">
                            <option value="published" selected>发表</option>
...
<form id="form_search" class="form-inline col-xs-12" role="form" style="margin-bottom:20px;">
    <div class="form-group">
        <label class="sr-only" for="input_category">分类</label>
    ...
    <div class="form-group">
        <label class="sr-only" for="input_search">关键字</label>
        <input type="text" name="search" value="<?php echo isset($keyword)? $keyword: ''; ?>" id="input_search" class="input-medium search-query form-control" placeholder="关键字" />
    </div>
    <div class="form-group">
        ...
        <?php
            if($allow_navigate_backend){
                echo '<a href="" class="btn btn-default add_record" data-toggle="modal" data-target="#quickwrite-modal"><i class="glyphicon glyphicon-plus"></i> 添加</a>'.PHP_EOL;
            }
            if($have_edit_template_privilege){
                echo '<a href="{{ module_site_url }}blog/template_config" class="btn btn-default add_record"><i class="glyphicon glyphicon-cog"></i> 编辑记录模板</a>'.PHP_EOL;
            }
        ?>
    </div>
</form>
        ...
        if($first_data != NULL){
            ...
            if($allow_navigate_backend){
                ...
                if($is_super_admin || $is_blog_editor || $is_blog_author || ($is_blog_contributor && $article['author_user_id'] == $user_id)){
                    echo '<a href="'.$backend_url.'/edit/'.$article['id'].'" class="btn btn-default edit_record" primary_key = "'.$article['id'].'">编辑</a>';
                    echo '&nbsp;';
                    echo '<a href="'.$backend_url.'/delete/'.$article['id'].'" class="btn btn-danger delete_record" primary_key = "'.$article['id'].'">删除</a>';
                }
                echo '</div>';
            }
            ...
            foreach($article['comments'] as $comment){
                ...
                echo '</div>';
                echo '<div>';
                echo '<a id="reply_comment_link_'.$comment['comment_id'].'" class="reply_comment_link" href="#" comment_id="'.$comment['comment_id'].'">回复</a>';
                echo '<a id="reply_cancel_link_'.$comment['comment_id'].'" class="reply_cancel_link" style="display:none;" href="#" comment_id="'.$comment['comment_id'].'">取消</a>';
                echo '</div>';
            ...
            if($article['allow_comment']){
                ...
                if(!$is_user_login){
                    ...
                    echo form_label('名字', ' for="" class="control-label col-sm-2');
                    echo '<div class="col-sm-8">';
                    echo form_input($secret_code.'xname', $name,
                        'id="'.$secret_code.'xname" placeholder="你的名字" class="form-control"');
                    echo '</div>';
                    echo '</div>';

                    echo '<div class="form-group">';
                    echo form_label('邮箱', ' for="" class="control-label col-sm-2');
                    echo '<div class="col-sm-8">';
                    echo form_input($secret_code.'xemail', $email,
                        'id="'.$secret_code.'xemail" placeholder="您的邮箱地址" class="form-control"');
                    echo '</div>';
                    echo '</div>';
                }
                echo '<div class="form-group">';
                echo form_label('网站', ' for="" class="control-label col-sm-2');
                echo '<div class="col-sm-8">';
                echo form_input($secret_code.'xwebsite', $website,
                    'id="'.$secret_code.'xwebsite" placeholder="您的网站" class="form-control"');
                echo '</div>';
                echo '</div>';

                echo '<div class="form-group">';
                echo form_label('评论', ' for="" class="control-label col-sm-2');
                echo '<div class="col-sm-8">';
                echo form_textarea($secret_code.'xcontent', $content,
                    'id="'.$secret_code.'xcontent" placeholder="您的评论" class="form-control text-area-comment"');
                echo '</div>';
                echo '</div>';

                echo '<div class="form-group"><div class="col-sm-offset-2 col-sm-8">';
                echo form_submit('submit', '评论', 'class="btn btn-primary"');
                echo '</div></div>';
                echo '</div>'; // end of comment-box
            }
            echo '</div>';
        }else if(isset($article) && $article == FALSE){
            echo '<div class="alert alert-danger">没有发现文章</div>';
        }
    ?>
</div>
...
<div id="record_content_bottom" class="alert alert-success">页面结束</div>
...
<script type="text/javascript">
    ...
    var LOAD_MESSAGE = '加载更多文章 &nbsp;<img src="{{ BASE_URL }}assets/nocms/images/ajax-loader.gif" />';
    var REACH_END_MESSAGE  = '没有更多文章显示';
