    ...
    public function _callback_column_widget_name($value, $row)
    {
        ...
        if (isset($_SESSION['__mark_move_widget_id'])) {
            ...
            if ($row->widget_id == $mark_move_widget_id) {
                // cancel link
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_widget/widget_move_cancel').'"><i class="glyphicon glyphicon-repeat"></i> 撤消</a>';
            } else {
                // paste before, paste after, paste inside
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_widget/widget_move_before/'.$row->widget_id).'"><i class="glyphicon glyphicon-open"></i> 放在前面</a>';
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_widget/widget_move_after/'.$row->widget_id).'"><i class="glyphicon glyphicon-save"></i> 放在后面</a>';
            }
        } else {
            $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_widget/widget_mark_move/'.$row->widget_id).'"><i class="glyphicon glyphicon-share-alt"></i> 移动</a>';
        }
