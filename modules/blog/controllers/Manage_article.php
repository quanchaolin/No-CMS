...
class Manage_article extends CMS_CRUD_Controller {
    ...
    protected function make_crud(){
        ...
        $crud->display_as('article_title','文章标题');
        $crud->display_as('article_url','Url地址 (默认留空)');
        $crud->display_as('keyword','关键字元数据(逗号分隔)');
        $crud->display_as('description','描述性元数据');
        $crud->display_as('date','创建日期');
        $crud->display_as('author_user_id','作者');
        $crud->display_as('content','内容');
        $crud->display_as('allow_comment','允许发表评论');
        $crud->display_as('status','状态');
        $crud->display_as('visited','访问量');
        $crud->display_as('featured','特定的');
        $crud->display_as('publish_date','发行日期');
        $crud->display_as('photo',' 照片');
        $crud->display_as('comments','评论');
        $crud->display_as('category_article','分类');
        ...
        $crud->set_tab_glyphicons(array(
                '照片'    => 'glyphicon-picture',
                '设置'   => 'glyphicon-th-list',
                '评论'  => 'glyphicon-comment',
            ));
