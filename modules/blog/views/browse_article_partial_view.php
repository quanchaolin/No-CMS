...
foreach($articles as $article){
    ...
    if($allow_navigate_backend){
        if($is_super_admin || $is_blog_editor || $is_blog_author || ($is_blog_contributor && $article->author_user_id == $user_id)){
            $article_backend_url .= '&nbsp;';
            $article_backend_url .= '<a href="'.$backend_url.'/edit/'.$article->id.'" class="btn btn-default edit_record" primary_key = "'.$article->id.'">编辑</a>';
            $article_backend_url .= '&nbsp;';
            $article_backend_url .= '<a href="'.$backend_url.'/delete/'.$article->id.'" class="btn btn-danger delete_record" primary_key = "'.$article->id.'">删除</a>';
        }
    }
