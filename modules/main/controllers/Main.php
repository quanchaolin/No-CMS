...
class Main extends CMS_Controller
{
    ...
    {
        $this->cms_guard_page('main_school');
        $navigation = $this->cms_navigation('main_school');
        $data = array();
        if($navigation->is_static != 1){
            $data['submenu_screen'] = $this->cms_submenu_screen('main_school');
        }
        $this->view('main/main_school', $data, 'main_school');
    }
    ...
    public function widget_top_nav($caption = 'Complete Menu', $first = true, $no_complete_menu = false, $no_quicklink = false, $navbar_class = 'navbar-default navbar-fixed-top', $navigations = null, &$notif = array())
    {
        ...
        if (!$no_complete_menu) {
            ...
            if($first && $this->cms_user_is_super_admin()){
                ...
                $result .= '<li><a href="{{ site_url }}factory_reset?from='.$this->cms_get_origin_uri_string().'"><i class="glyphicon glyphicon-repeat"></i> 恢复出厂设置</a></li>';
            }
            $result .= '</ul>';
        }
            ...
            if ($this->cms_user_is_super_admin()) {
                if ($this->cms_editing_mode()) {
                    $toggle_editing = '<a id="__toggle_editing_off" href="#" class="hidden-xs" style="font-size:small;">'.
                            '<i class="glyphicon glyphicon-eye-open"></i> 切换视图'.
                        '</a>';
                    $edit_menu = '<li class="dropdown hidden-xs" id="__edit_menu">
                        <a class="dropdown-toggle" data-toggle="dropdown" href="http://localtest.me/No-CMS/main/management" style="padding-left: 15px; padding-right: 10px; font-size:small;"><i class="glyphicon glyphicon-pencil"></i> 编辑 <span class="caret"></span></a>
                        <ul class="dropdown-menu">
                            <li class="dropdown">
                                <a href="{{ site_url }}main/manage_navigation?from='.$this->cms_get_origin_uri_string().'">导航</a>
                            </li>
                            <li class="dropdown">
                                <a href="{{ site_url }}main/manage_quicklink?from='.$this->cms_get_origin_uri_string().'">快速链接</a>
                            </li>
                            <li class="dropdown">
                                <a id="__editing_section_top_fix_link" href="#">顶部</a>
                            </li>
                            <li role="separator" class="divider"></li>
                            <li><a href="{{ site_url }}factory_reset?from='.$this->cms_get_origin_uri_string().'"><i class="glyphicon glyphicon-repeat"></i> 恢复出厂设置</a></li>
                        </ul>
                    </li>';
                } else {
                    $toggle_editing = '<a id="__toggle_editing_on" href="#" class="hidden-xs" style="font-size:small;">'.
                            '<i class="glyphicon glyphicon-pencil"></i> 切换编辑'.
                        '</a>';
                }
            }
