...
/**
 * Description of Manage_vehicle_maintenance
 *
 * @author No-CMS Module Generator
 */
class Manage_vehicle_maintenance extends CMS_CRUD_Controller {

    protected $URL_MAP = array();
    protected $TABLE_NAME = 'main_vehicle_maintenance';
    protected $COLUMN_NAMES = array('vehicle_id', 'photos', 'description', 'projects', 'pubdate', 'address', 'charges',  'brokerage', 'status');
    protected $PRIMARY_KEY = 'vehicle_maintenance_id';
    ...
    protected function make_crud(){
        ...
        // set subject
        $crud->set_subject('车辆维护');

        // displayed columns on list, edit, and add, uncomment to use
        $crud->columns('vehicle_id','projects','charges', 'description','pubdate','status');
        $crud->edit_fields('vehicle_id', 'photos', 'description', 'projects', 'pubdate', 'address', 'charges',  'brokerage', 'status', '_updated_by', '_updated_at');
        $crud->add_fields('vehicle_id', 'photos', 'description', 'projects', 'pubdate', 'address', 'charges',  'brokerage', 'status', '_created_by', '_created_at');
        //$crud->set_read_fields('vehicle_num', 'vehicle_brand', 'vehicle_mode','vehicle_type');

        // caption of each columns
        $crud->display_as('vehicle_id','车牌号');
        $crud->display_as('photos','照片');
        $crud->display_as('description','描述');
        $crud->display_as('projects','保养项目');
        $crud->display_as('pubdate','保养日期');
        $crud->display_as('address','地址');
        $crud->display_as('charges','费用');
        $crud->display_as('brokerage','经手人');
        $crud->display_as('status','审核状态');
        ...
        ////////////////////////////////////////////////////////////////////////
        $crud->required_fields('vehicle_id','description');
        ...
        $crud->set_field_half_width(array('vehicle_id', 'projects','pubdate', 'charges','brokerage'));
        $crud->set_relation('vehicle_id', cms_table_name('main_vehicle'), 'vehicle_num');
        ////////////////////////////////////////////////////////////////////////
    ...
    public function index(){
        ...
        // show the view
        $this->view($this->cms_module_path().'/Manage_vehicle_maintenance_view', $output,
            $this->n('main_vehicle_maintenance_management'), $config);
    }
