...
class Manage_quicklink extends CMS_CRUD_Controller {
    ...
    protected function make_crud(){
        ...
        $crud->display_as('navigation_id','导航名称');
    ...
    public function _callback_column_navigation_id($value, $row)
    {
        ...
        if (isset($_SESSION['__mark_move_quicklink_id'])) {
            ...
            if ($row->quicklink_id == $mark_move_quicklink_id) {
                // cancel link
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_quicklink/quicklink_move_cancel').'"><i class="glyphicon glyphicon-repeat"></i> 撤消</a>';
            } else {
                // paste before, paste after, paste inside
                $html .= '<br /><a href="'.site_url($this->cms_module_path().'/manage_quicklink/quicklink_move_before/'.$row->quicklink_id).'"><i class="glyphicon glyphicon-open"></i> 放在前面</a>';
                $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_quicklink/quicklink_move_after/'.$row->quicklink_id).'"><i class="glyphicon glyphicon-save"></i> 放在后面</a>';
            }
        } else {
            $html .= ' | <a href="'.site_url($this->cms_module_path().'/manage_quicklink/quicklink_mark_move/'.$row->quicklink_id).'"><i class="glyphicon glyphicon-share-alt"></i> 移动</a>';
        }
